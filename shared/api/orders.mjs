...
export function normalizeOrderListRow(row = {}) {
  return {
    ...
    billing_name: String(row.billing_name || row.buyer_name || "").trim(),
    shipping_name: String(row.shipping_name || "").trim(),
    recipient_name: String(row.recipient_name || row.shipping_name || "").trim(),
    buyer_email: String(row.buyer_email || "").trim(),
    billing_email: String(row.billing_email || row.buyer_email || "").trim(),
    billing_address: String(row.billing_address || "").trim(),
...
export function normalizeOrderDetail(order = {}) {
  return {
    ...
    billing_name: String(order.billing_name || order.buyer_name || "").trim(),
    shipping_name: String(order.shipping_name || "").trim(),
    recipient_name: String(order.recipient_name || order.shipping_name || "").trim(),
    buyer_email: String(order.buyer_email || "").trim(),
    billing_email: String(order.billing_email || order.buyer_email || "").trim(),
    billing_address: String(order.billing_address || "").trim(),
